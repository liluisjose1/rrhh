...
$sql = "SELECT CONCAT(p.nombres,' ',p.apellidos) as nombre, i.f_desde, i.f_hasta, DATEDIFF(i.f_hasta,i.f_desde) dias FROM `permisos` as i, personal p WHERE i.id_personal=p.codigo AND i.f_desde BETWEEN '$f_desde' AND '$f_hasta' AND i.id_personal='$id_personal' ORDER BY p.id_area";
$resultado = $conexion->query($sql);

$sql1 = "SELECT CONCAT(p.nombres,' ',p.apellidos) AS nombre,a.nombre as area,SUM(DATEDIFF(i.f_hasta,i.f_desde)) total FROM permisos as i, personal as p, areas as a WHERE i.f_desde BETWEEN '$f_desde' AND '$f_hasta' AND i.id_personal = p.codigo AND p.id_area=a.id_area AND i.id_personal='$id_personal'";
