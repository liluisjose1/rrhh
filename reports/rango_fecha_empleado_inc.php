...
$sql = "SELECT CONCAT(p.nombres,' ',p.apellidos) as nombre, i.f_desde, i.f_hasta, DATEDIFF(i.f_hasta,i.f_desde) dias FROM `incapacidad` as i, personal p WHERE i.id_personal=p.codigo AND i.f_desde BETWEEN '$f_desde' AND '$f_hasta' ORDER BY p.id_area";
$resultado = $conexion->query($sql);

$sql1 = "SELECT CONCAT(p.nombres,' ',p.apellidos) AS nombre,a.nombre as area,SUM(DATEDIFF(i.f_hasta,i.f_desde)) total FROM incapacidad as i, personal as p, areas as a WHERE i.f_desde BETWEEN '$f_desde' AND '$f_hasta' AND i.id_personal = p.codigo AND p.id_area=a.id_area AND i.id_personal='$id_personal'";
...
class PDF extends FPDF
{
    function Header()
    {
        ...
        $this->Cell(0,0, 'Reporte de Incapacidad de Personal',0,1,'C');
        $this->Ln(5);
